...
class RenderFont(object):
    ...
    def render_sample(self,font,mask,font_2,mask_2):
        """
        ...
        H_2,W_2 = self.robust_HW(mask_2)
        f_asp = self.font_state.get_aspect_ratio(font)
        f_asp_2 = self.font_state.get_aspect_ratio(font_2)

        # find the maximum height in pixels:
        max_font_h = min(0.9*H, (1/f_asp)*W/(self.min_nchar+1))
        max_font_h_2 = min(0.9*H_2, (1/f_asp_2)*W_2/(self.min_nchar+1))
        max_font_h = min(max_font_h, self.max_font_h)
        max_font_h_2 = min(max_font_h_2, self.max_font_h)
        if max_font_h < self.min_font_h: # not possible to place any text here
            return #None
        if max_font_h_2 < self.min_font_h: # not possible to place any text here
            return #None
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h \
              and max_font_h_2 > self.min_font_h:
            # if i > 0:
            ...
            f_h_px_2 = self.sample_font_height_px(self.min_font_h, max_font_h_2)
            ...
            f_h_2 = self.font_state.get_font_size(font_2, f_h_px_2)

            # update for the loop
            max_font_h = f_h_px
            max_font_h_2 = f_h_px_2
            i += 1

            font.size = f_h # set the font-size
            font_2.size = f_h_2 # set the font-size

            # compute the max-number of lines/chars-per-line:
            nline,nchar = self.get_nline_nchar(mask.shape[:2],f_h,f_h*f_asp)
            nline_2,nchar_2 = self.get_nline_nchar(mask_2.shape[:2],f_h_2,f_h_2*f_asp_2)
            #print "  > nline = %d, nchar = %d"%(nline, nchar)

            assert nline >= 1 and nchar >= self.min_nchar
            assert nline_2 >= 1 and nchar_2 >= self.min_nchar

            # sample text:
            text_type = sample_weighted(self.p_text)
            text_type_2 = sample_weighted(self.p_text)
            ...
            txt_arr_2,txt_2,bb_2 = self.render_curved(font_2, text)
            bb = self.bb_xywh2coords(bb)
            bb_2 = self.bb_xywh2coords(bb_2)
            ...
            if np.any(np.r_[txt_arr.shape[:2]] > np.r_[mask.shape[:2]]):
                ...
                continue
            if np.any(np.r_[txt_arr_2.shape[:2]] > np.r_[mask_2.shape[:2]]):
                #warn("text-array is bigger than mask")
                continue

            # position the text within the mask:
            text_mask,loc,bb, _ = self.place_text([txt_arr], mask, [bb])
            text_mask_2,loc_2,bb_2, _ = self.place_text([txt_arr_2], mask_2, [bb_2])
            if len(loc) > 0 and len(loc_2) > 0:#successful in placing the text collision-free:
                return text_mask,loc[0],bb[0],text, \
                    text_mask_2,loc_2[0],bb_2[0],text
        return #None
