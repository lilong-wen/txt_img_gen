...
import deepthCamera as su
import render_text as tu
...
class Renderer(object):
    ...
    def render_text(self,dict_1, dict_2,ninstance=1,viz=False):
        """
        ...
        """
        rgb,depth,seg,area,label = dict_1['img'], dict_1['depth'],\
            dict_1['seg'], dict_1['area'], dict_1['label']
        rgb_2,depth_2,seg_2,area_2,label_2 = dict_2['img'], dict_2['depth'],\
            dict_2['seg'], dict_2['area'], dict_2['label']

        try:
            # depth -> xyz
            xyz = su.DepthCamera.depth2xyz(depth)
            xyz_2 = su.DepthCamera.depth2xyz(depth_2)
            # find text-regions:
            regions = TextRegions.get_regions(xyz,seg,area,label)
            regions_2 = TextRegions.get_regions(xyz_2,seg_2,area_2,label_2)

            # find the placement mask and homographies:
            regions = self.filter_for_placement(xyz,seg,regions)
            regions_2 = self.filter_for_placement(xyz_2,seg_2,regions_2)

            # finally place some text:
            nregions = len(regions['place_mask'])
            nregions_2 = len(regions_2['place_mask'])
            if nregions < 1: # no good region to place text on
                return []
            if nregions_2 < 1: # no good region to place text on
                return []
        ...
        for i in range(ninstance):
            ...
            place_masks_2 = copy.deepcopy(regions_2['place_mask'])

            print(colorize(Color.CYAN, " ** instance # : %d"%i))

            idict = {'img':[], 'charBB':None, 'wordBB':None, 'txt':None}
            idict_2 = {'img':[], 'charBB':None, 'wordBB':None, 'txt':None}

            m = self.get_num_text_regions(nregions)#np.arange(nregions)#min(nregions, 5*ninstance*self.max_text_regions))
            m_2 = self.get_num_text_regions(nregions_2)#np.arange(nregions)#min(nregions, 5*ninstance*self.max_text_regions))
            reg_idx = np.arange(min(2*m,nregions))
            reg_idx_2 = np.arange(min(2*m_2,nregions_2))
            np.random.shuffle(reg_idx)
            np.random.shuffle(reg_idx_2)
            reg_idx = reg_idx[:m]
            reg_idx_2 = reg_idx_2[:m_2]
            ...
            num_txt_regions_2 = len(reg_idx_2)
            NUM_REP = 5 # re-use each region three times:
            reg_range = np.arange(NUM_REP * num_txt_regions) % num_txt_regions
            reg_range_2 = np.arange(NUM_REP * num_txt_regions_2) % num_txt_regions_2

            for idx,idx2 in zip(reg_range, reg_range_2):
                ireg = reg_idx[idx]
                ireg2 = reg_idx_2[idx2]
                try:
                    if self.max_time is None:
                        txt_render_res = self.place_text(img,place_masks[ireg],
                                                         regions['homography'][ireg],
                                                         regions['homography_inv'][ireg],
                                                         img_2, place_masks_2[ireg2],
                                                         regions_2['homography'][ireg2],
                                                         regions_2['homography_inv'][ireg2])
                    else:
                ...
                if txt_render_res is not None:
                    placed = True
                    img,text,bb,collision_mask,img_2,text_2,bb_2,collision_mask_2 = txt_render_res
                    # update the region collision mask:
                    place_masks[ireg] = collision_mask
                    place_masks_2[ireg2] = collision_mask_2
                    # store the result:
                    itext.append(text)
                    itext_2.append(text_2)
                    ibb.append(bb)
                    ibb_2.append(bb_2)
            ...
            if placed:
                ...
                idict_2['img'] = img_2
                idict_2['txt'] = itext_2
                idict_2['charBB'] = np.concatenate(ibb_2, axis=2)
                idict_2['wordBB'] = self.char2wordBB(idict_2['charBB'].copy(), ' '.join(itext_2))
                res.append(idict.copy())
                res_2.append(idict_2.copy())
                if viz:
                    viz_textbb(1,img, [idict['wordBB']], alpha=1.0)
                    viz_textbb(1,img_2, [idict_2['wordBB']], alpha=1.0)
                    viz_masks(2,img,seg,depth,regions['label'])
                    viz_masks(2,img_2,seg_2,depth_2,regions_2['label'])
